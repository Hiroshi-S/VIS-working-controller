...
{
    public partial class ExercisePlanner : System.Web.UI.UserControl
    {
        ...
        protected void CheckExistingPlan()
        {
            try
            {
                ...
                //delete all 0's if there is any
                var cepZeros = (from ceps in cvdc.TrainingDiaries
                                where ceps.iCustomerId == Convert.ToInt32(Session["MemberNo"])
                                where ceps.iExValue == 0
                                where ceps.bFromVOS == true
                                select ceps);
               
                cvdc.TrainingDiaries.DeleteAllOnSubmit(cepZeros);
                ...
                foreach (TrainingDiary td in cep)
                {
                    ...
                    if (iday != td.iWeekDay)
                    {
                        ...
                        iday = td.iWeekDay;
                        imgbutton = "<img src=\"/images/icons/web/plus.jpg\" title=\"Add more exercise to " + sday + "\" onclick=\"addTrainingDiaryRow(0," + td.iWeekDay + ", 'planner');return false;\"/>";
                    ...
                    LitExPlanner.Text += "<tr id='" + td.iId + "00'><td style=\"font-weight: bold;\">" + sday + "</td><td>" + imgbutton + "</td>" +
                                            ...
                                            "<td>" + intensddl + "</td>" +
                                            "<td>" + durddl + "</td>" +
                                            "<td><img title='Delete this exercise' id='del-" + td.iId + "' onclick=\"deleteTrainingRow(" + td.iId + ", " + td.iWeekNumber + ", " + td.iWeekDay + ");return false;\" src=\"/images/icons/web/delete.png\" width=\"18px\"/></td></tr>";
                }
                        ...
                        case 1: //beginner
                            {
                                ...
                                wprogramchosen = "<h3>Beginner Program</h3>";
                        ...
                        case 2://intermediate
                            {
                                ...
                                wprogramchosen = "<h3>Intermediate Program</h3>";
                        ...
                        case 3:
                            {
                                ...
                                wprogramchosen = "<h3>Advanced Program</h3>";
        ...
        protected void UpdateTrainingSummary()
        {
            try
            {
                int aHardCardio = TrainingDiaryScreen.CalculateActualHardCardio(0, Convert.ToInt32(Session["MemberNo"]), "VVT", DateTime.Today);
                int aTotalCardio = TrainingDiaryScreen.CalculateActualTotalCardio(0, Convert.ToInt32(Session["MemberNo"]), "VVT", DateTime.Today);
                int aLMCardio = aTotalCardio - aHardCardio;
                int aWeights = TrainingDiaryScreen.CalculateActualWeights(0, Convert.ToInt32(Session["MemberNo"]), "VVT", DateTime.Today);
        ...
        protected void MyGoalImagebuttonNextClick(object sender, ImageClickEventArgs e)
        {
            ...
            if (checksum != null && checksum.bHardCardioAchieved && checksum.bTotCardioAchieved && checksum.bWeightsAchieved && checkProg.Any()) //if they dont have program, lock them in 
            {
                ...
                //delete the 0's training diary
                var zeroTraining = (from ztt in cvdc.TrainingDiaries
                                    where ztt.iCustomerId == (int) Session["MemberNo"]
                                    where ztt.bFromVOS == true
                                    where ztt.iExValue == 0
                                    select ztt);

                cvdc.TrainingDiaries.DeleteAllOnSubmit(zeroTraining);

                cvdc.SubmitChanges();
                cvdc.Dispose();
                Label1.Text = "Success";
                Response.Redirect("/club-vision/account-setup/eating-planner/", false);
            }
            else
            {
                Label1.Text = "";
